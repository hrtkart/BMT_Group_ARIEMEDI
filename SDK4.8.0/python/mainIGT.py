...
class TrackingUpdate(threading.Thread):
    ...
    def run(self):
        ...
        while(connectflag):
            ...
            print("Gravity vector is ", gravityVector.x, gravityVector.y, gravityVector.z)
            ...
            for i in range(n.value):
                print(allMarkers[i].x, allMarkers[i].y, allMarkers[i].z)
            ...
            for i in range(armdSteroTracking.getTrackingToolCurrentNum()):
                ...
                if currentTool.matchStatus == 1:
                    ...
                    if (i == 0):
                        ...
                        message = pyigtl.TransformMessage(device_name=name.value.decode(), matrix=matrix)
                        message.pack()
                        igtserver.send_message(message, False)
                    else:
                        ...
                        message = pyigtl.TransformMessage(device_name=name.value.decode(), matrix=projectedMatrix)
                        message.pack()
                        igtserver.send_message(message, False)
                else:
...
if __name__ == '__main__':
    ...
    if connectStatue == 0:
        ...
        # create IGT link to send tracking message
        igtserver = pyigtl.OpenIGTLinkServer(18944)
        while(not igtserver.is_connected()):
            continue
        ...
        # start tracking process and thread
