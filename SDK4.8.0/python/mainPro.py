...
class PointInf(ctypes.Structure):
    _fields_ = [("x", ctypes.c_float), ("y", ctypes.c_float), ("z", ctypes.c_float), ("r", ctypes.c_float), ("g", ctypes.c_float), ("b", ctypes.c_float)]
...
# monitoring update thread
class MonitoringUpdate(threading.Thread):
    def __init__(self, thread_name):
        super(MonitoringUpdate, self).__init__(name = thread_name)

    def run(self):
        global connectflag
        global imageC

        # get visible image size
        VMImageSize = (ctypes.c_int * 2)(0, 0)
        armdSteroTracking.getMonitorImageSize(VMImageSize)
        VMImageW = VMImageSize[0]
        VMImageH = VMImageSize[1]

        while(connectflag):
            # update tracking information
            armdSteroTracking.monitoringUpdate()

            # get infrared imaging data
            centerImg = armdSteroTracking.getMonitorImagingData()

            centerImgArray = np.ctypeslib.as_array(centerImg, shape=(VMImageH, VMImageW, 3))
            imageC = np.ascontiguousarray(np.copy(centerImgArray), dtype=np.uint8)
...
def functionsetting():
    ...
    # monitoring
    armdSteroTracking.getMonitorImagingData.restype = ctypes.POINTER(ctypes.c_ubyte)
    armdSteroTracking.getMonitorImageSize.argtypes = [ctypes.POINTER(ctypes.c_int)]

    # reconstruction
    armdSteroTracking.getPointCloud.restype = PointInf
    armdSteroTracking.getDepthImageSize.argtypes = [ctypes.POINTER(ctypes.c_int)]
    armdSteroTracking.getDepthImage.argtypes = [ctypes.POINTER(ctypes.c_float)]
    armdSteroTracking.getTextureImageUChar.restype = ctypes.POINTER(ctypes.c_ubyte)

if __name__ == '__main__':
    # load the C++ dll
    armdSteroTracking = ctypes.cdll.LoadLibrary("./ARMDCSDLLWarpperPro.dll")
    ...
    if connectStatue == 0:
        ...
        print(device.firmwareVersionH, device.firmwareVersionM, device.firmwareVersionL)
        ...
        # start monitoring process and thread
        armdSteroTracking.startMonitoring(False)
        MonitoringUpdate("monitoringUpdate").start()

        # show visible imaging data
        showVMImage = False
        if(showVMImage):
            import matplotlib.pyplot as plt
            VMImageSize = (ctypes.c_int * 2)(0, 0)
            armdSteroTracking.getIFImageSize(VMImageSize)
            VMImageW = VMImageSize[0]
            VMImageH = VMImageSize[1]
            imageC = np.zeros((VMImageH, VMImageW, 3))

            figVM = plt.figure()
            fig = figVM.add_subplot(1, 1, 1)
            while(True):
                plt.pause(0.05)
                fig.imshow(imageC)

        # reconstruction after 10 seconds
        time.sleep(5)
        suspend = True
        armdSteroTracking.reconstruction(360, 363, 776, 418, 776, 418)
        suspend = False
        armdSteroTracking.beep(40)
        pcnum = armdSteroTracking.getPointNum()
        pcpath = b'./pc.ply'
        armdSteroTracking.savePointCloud(pcpath, True)
        print("Reconstruction point num is ", pcnum)
        # print the first point
        if (pcnum > 0):
            point = armdSteroTracking.getPointCloud(0)
            print("The first point position is (", point.x,point.y,point.z,") , and the color is ",point.r,point.g,point.b)

        # show depth and texture data of reconstruction
        showDTImage = False
        if (showDTImage):
            import matplotlib.pyplot as plt
            # depth image
            DImageSize = (ctypes.c_int * 2)(0, 0)
            armdSteroTracking.getDepthImageSize(DImageSize)
            DImageW = DImageSize[0]
            DImageH = DImageSize[1]
            depthImg = (ctypes.c_float * (DImageH * DImageW))()
            armdSteroTracking.getDepthImage(depthImg)
            depthImgArray = np.ctypeslib.as_array(depthImg)*(255.0/3000)
            imageD = depthImgArray.astype(np.uint8).reshape(DImageH, DImageW)

            # texture image
            texyureImg = armdSteroTracking.getTextureImageUChar()
            textureImgArray = np.ctypeslib.as_array(texyureImg, shape=(1200, 1920, 3))
            imageT = np.ascontiguousarray(np.copy(textureImgArray), dtype=np.uint8)

            figDT = plt.figure()
            fig1 = figDT.add_subplot(1, 2, 1)
            fig2 = figDT.add_subplot(1, 2, 2)
            while (True):
                plt.pause(0.05)
                fig1.imshow(imageD, cmap='gray')
                fig2.imshow(imageT[:,:,::-1])
        armdSteroTracking.saveLog(b'./record.log')
    else:
        print(armdSteroTracking.getConnectionStatus())
